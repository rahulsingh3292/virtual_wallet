...
class HomePage(View):
  def get(self,request: HttpRequest):
    ...
    users: "QuerySet[User]" = User.objects.filter(is_superuser=False).exclude(id=request.user.id)
...
class PaymentsRequestPage(View):
  def get(self, request):
    payment_requests: "QuerySet[PaymentRequest]" = PaymentRequest.objects.filter(to_user=request.user) 
    return render(
...
class UserTransactionsPage(View):
  def get(self, request: HttpRequest):
    transactions: "QuerySet[Transaction]" = Transaction.objects.filter(user=request.user)
